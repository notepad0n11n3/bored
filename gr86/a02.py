...
# --- Modelado predictivo con XGBoost (Top-N predicciones) ---
def predict_next(df, top_n=5):
    X = np.array(df["vector"].tolist())[:-1]
    y = np.array(df["vector"].tolist())[1:]  # Predecir la combinación siguiente

    model = MultiOutputClassifier(XGBClassifier(tree_method="hist", eval_metric="logloss", use_label_encoder=False))
    model.fit(X, y)

    last_vector = X[-1].reshape(1, -1)
    proba = model.predict_proba(last_vector)

    # Obtener probabilidad de que cada número aparezca (p1 si existe, sino 0)
    probs = np.array([p[1] if len(p) > 1 else 0.0 for p in proba])
    top_indices = probs.argsort()[::-1][:top_n]

    pred_numbers = [str(i + 1).zfill(2) for i in top_indices]

    with open("MaybE.txt", "w") as f:
        f.write("Top {} predicciones (posiciones 1-50):\n".format(top_n))
...
def main():
    ...
    print("[+] Entrenando modelo y generando predicciones...")
    predict_next(df, top_n=10)

    print("[✓] Predicciones guardadas en MaybE.txt")
